...
export async function changeAdminPasswordAction(formData: FormData) {
  ...
  const res = await db.execute('SELECT * FROM admin WHERE id = 1');
  ...
  await db.execute({
    sql: 'UPDATE admin SET password_hash=?, salt=? WHERE id=1',
...
export async function createTokenAction(formData: FormData) {
  ...
  const token = await setAccountToken(accountId);
  revalidatePath('/admin');
...
export async function resetAllTokensAction() {
  ...
  for (const a of accounts) await setAccountToken(a.id);
  revalidatePath('/admin');
  return { ok: true };
}
...
export async function deletePairAction(formData: FormData) {
  if (!(await isAdmin())) return { ok: false, error: 'Tidak berwenang.' };
  await deletePair(Number(formData.get('id')));
  revalidatePath('/');
  revalidatePath('/admin');
  revalidatePath('/scoreboard');
  return { ok: true };
...
export async function toggleNewsFlagAction(formData: FormData) {
  ...
  if (flag === 'featured') {
    ...
    const db = await ensureDb();
    await db.execute('UPDATE news SET featured = 0');
    await db.execute({
      sql: 'UPDATE news SET featured = 1 WHERE id = ?',
      args: [id],
    });
  } else {
    await upsertNews({ id, title: news.title, body: news.body, cover_url: news.cover_url, featured: news.featured, is_new: news.is_new ? 0 : 1, published: news.published });
  }
  revalidatePath('/');
  revalidatePath('/admin');
  revalidatePath('/berita');
...
export async function castVoteAction(formData: FormData) {
  ...
  const res = await castVote(session.accountId, pairId, session.voterId);
...
export async function generateVotersAction(formData: FormData) {
  ...
  const res = await generateVotersBatch(fromNo, toNo);
  revalidatePath('/admin');
...
export async function setScoreboardPublishedAction(formData: FormData) {
  if (!(await isAdmin())) return { ok: false, error: 'Tidak berwenang.' };
  await setScoreboardPublished(formData.get('published') === 'on');
  revalidatePath('/scoreboard');
  revalidatePath('/admin');
  return { ok: true };
}
...
export async function setScoreboardRevealedAction(formData: FormData) {
  ...
  await setScoreboardRevealed(revealed);
  revalidatePath('/scoreboard');
  revalidatePath('/admin');
  return { ok: true };
}
...
export async function setVotingOpenAction(formData: FormData) {
  if (!(await isAdmin())) return { ok: false, error: 'Tidak berwenang.' };
  await setVotingOpen(formData.get('open') === 'on');
  revalidatePath('/vote');
  revalidatePath('/admin');
  return { ok: true };
}
